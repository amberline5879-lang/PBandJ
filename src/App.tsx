...
function AppContent() {
  const [hasOnboarded, setHasOnboarded] = React.useState(() => {
    return localStorage.getItem('serene_has_onboarded') === 'true';
  });

  const { user } = useAuth();

  // Watch for dynamic updates like logout which clear onboarding state
  const isCurrentlyOnboarded = localStorage.getItem('serene_has_onboarded') === 'true';

  if (!isCurrentlyOnboarded) {
    return (
      <Onboarding
        onComplete={() => {
          localStorage.setItem('serene_has_onboarded', 'true');

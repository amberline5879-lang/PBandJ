...
const AICoach: React.FC = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      console.warn("Express AI endpoint unavailable, checking client fallback:", error);
      ...
      if (clientApiKey) {
        try {
          ...
          const aiText = response.text || "I'm sorry, I couldn't quite process that. Could you try again?";
          setMessages(prev => [...prev, { role: 'model', text: aiText }]);
          return;
        } catch (clientErr) {
      ...
      setMessages(prev => [...prev, { 
        role: 'model', 
        text: "I am running in static frontend mode (e.g., GitHub Pages). To enable live AI chat, either deploy the full-stack server (e.g. Render/Railway) or set `VITE_GEMINI_API_KEY` in your environment variables!" 
      }]);
    } finally {
      setIsLoading(false);
    }
